...
from .dataset_utils import PointCloudProjection
import numpy as np 
...
class VisualizeDataset:
    ...
    def plot_projected_points(data, all_points=True):
        ...
        # Get point cloud 
        point_cloud_uncalib = data['uncalibed_pcd']
        point_cloud_calib = data['point_cloud']

        # RGB image 
        image = data['image'] #.transpose(1, 2, 0)
    ...
    @staticmethod
    def plot_depth_image(data, i, cmap='viridis'):
        plt.figure(figsize=(16,16))
        plt.subplot(1,2,1)
        plt.imshow(data['depth_img'].squeeze(0).permute(1,2,0).numpy()[:,:,i], cmap=cmap)
        plt.title('Calibrated Depth Image')
        plt.subplot(1,2,2)
        plt.title('Un-alibrated Depth Image')
        plt.imshow(data['uncalibed_depth_img'].squeeze(0).permute(1,2,0).numpy()[:,:,i], cmap=cmap)
        plt.show()
