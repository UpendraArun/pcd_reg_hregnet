...
def val_reg(args, net, chamfer_loss, mi_loss, max_val_c_loss, epoch, val_result_dict):
    ...
    #return total_loss, total_l_trans, val_result_dict
...
def train_reg(args):
    ...
    net.load_state_dict(torch.load(args.pretrain_model_feats), strict=False)
    ...
    model_params = list(net.named_parameters()) + list(mi_loss.named_parameters())
    ...
    optimizer_net = torch.optim.AdamW([
        ...
        {"params": block_params, "lr": block_lr}], lr=base_lr, weight_decay=weight_decay)

    scheduler_net = OneCycleLR(
        optimizer_net,
        max_lr=[base_lr, block_lr],  # Max LR for different param groups
        total_steps=args.epochs * len(train_loader),  # Total training steps
        pct_start=0.08,  # 4% warm-up
        anneal_strategy="cos",  # Cosine decay
        div_factor=10.0,  # Initial LR = max_lr / div_factor
        final_div_factor=100.0  # Minimum LR = max_lr / final_div_factor
        )

    #optimizer_net = optim.Adam(model_params, lr=args.lr)
    #scheduler_net = StepLR(optimizer_net, step_size=200, gamma=0.5)

    # Create the CosineLR Scheduler with Warmup
    # scheduler_net = CosineLRScheduler(
    #     optimizer_net,
    #     t_initial=args.epochs*len(train_loader),   # Total number of epochs
    #     lr_min=1e-5,    # Minimum learning rate
    #     warmup_t=2*len(train_loader),     # Number of warmup epochs
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i, data in pbar:
            ...
            total_R_err += R_err_.detach()
            total_t_err += T_err_.detach()
            total_geod_dist += geodesic_dist.mean().item()
            total_eucl_dist += eucl_dist.mean().item()
            ...
            #if i % 10 == 0:
                #pbar.set_description('Train Epoch:{}[{}/{}({:.0f}%)]\tLoss: {:.6f}  \ttransformation loss:{:.6f}'.format(
                    #epoch+1, i, len(train_loader), 100. * i/len(train_loader), loss.item(), l_trans.item()))
        ...
        # total_val_loss, val_trans_loss, val_result_dict \
        #                                       = val_reg(args, net, chamfer_loss, mi_loss, max_val_c_loss, epoch, val_result_dict)
        torch.cuda.synchronize()
        torch.cuda.empty_cache()
        ...
        if args.use_wandb:
            wandb.log({"train/total loss":total_loss,
                       ...
                       "train/learning_rate_base": optimizer_net.param_groups[0]['lr'],
                        "train/learning_rate_block": optimizer_net.param_groups[1]['lr'],
