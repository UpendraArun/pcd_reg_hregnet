...
def val_reg(args, net, chamfer_loss):
    ...
    with torch.no_grad():
        ...
        for i, data in pbar: 
            ...
            total_loss += l_trans.item() + c_loss.item()
...
def train_reg(args):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i, data in pbar:
            ...
            #loss = JSEstimatorLoss(pos_pairs, neg_pairs)
            c_loss = alpha*chamfer_loss(ret_dict['src_feats']['xyz_3'], ret_dict['dst_feats']['xyz_3'])
            ...
            if i % 10 == 0:
                pbar.set_description('Train Epoch:{}[{}/{}({:.0f}%)]\tLoss: {:.6f} \tchamfer_loss:{:.6f} \ttransformation loss:{:.6f}'.format(
                    epoch+1, i, len(train_loader), 100. * i/len(train_loader), loss.item(), c_loss.item(), l_trans.item()    
                ))
        ...
        total_val_loss = val_reg(args, net, chamfer_loss)
        torch.cuda.empty_cache()
