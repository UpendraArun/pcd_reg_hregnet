...
def val_reg(args, net, mi_loss):
    ...
    with torch.no_grad():
        ...
        for i, data in pbar: 
            ...
            total_loss += l_trans.item() + js_loss.item()
...
def train_reg(args):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i, data in pbar:
            ...
            if i % 10 == 0:
                pbar.set_description('Train Epoch:{}[{}/{}({:.0f}%)]\tLoss: {:.6f} \tMI_loss:{:.6f} \ttransformation loss:{:.6f}'.format(
                    epoch+1, i, len(train_loader), 100. * i/len(train_loader), loss.item(), js_loss.item(), l_trans.item()    
                ))
        ...
        total_val_loss = val_reg(args, net, mi_loss)
        torch.cuda.empty_cache()
