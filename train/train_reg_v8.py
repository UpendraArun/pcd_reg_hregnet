# Training script
# Model V2 (fully unsupervised version)
# SVD head
# Loss = Ch + MI
...
def val_reg(val_loader, args, net, chamfer_loss, mi_loss, max_val_c_loss, epoch, val_result_dict):
    config = Config(args)
    ...
    if args.use_wandb:
        wandb.log({
                    ...
                    f"val/translation-error-xyz": total_t_err.mean().item(),
...
def train_reg(args):
    ...
    scheduler_net = StepLR(optimizer_net, step_size=15, gamma=0.5)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        total_val_loss, val_normalized_c_loss, val_c_loss, val_js_loss, max_val_c_loss, val_result_dict \
                                              = val_reg(val_loader, args, net, chamfer_loss, mi_loss, max_val_c_loss, epoch, val_result_dict)
        torch.cuda.synchronize()
        torch.cuda.empty_cache()
