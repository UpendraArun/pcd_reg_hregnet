# Training script
# Model V6 (fully unsupervised version)
# Loss = Ch + MI
...
def val_reg(args, net, chamfer_loss, mi_loss, max_val_c_loss, epoch, val_result_dict):
    ...
    with torch.no_grad():
        ...
        for i, data in pbar: 
            ...
            total_loss += + normalized_c_loss + js_loss.item()
...
def train_reg(args):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i, data in pbar:
            ...
            for idx in range(3):
                _, _, _, R_err_, geodesic_dist, T_err_, eucl_dist \
                                             = transformation_loss(ret_dict['rotation'][idx].detach(),\
                                                                ret_dict['translation'][idx].detach(),\
