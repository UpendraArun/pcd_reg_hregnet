...
class CalibEval(object):
    ...
    def get_results(self):
        ...
        sd_t, sd_r, sd_dR, sd_dT = self.getSD()
        ...
        self.results = { 
            ...
            #'translation': self.config.dataset_config.max_trans_error,
            #'rotation': self.config.dataset_config.max_rot_error,
            ...
            "mean_sd_dRT": [np.mean(sd_dR).tolist(), np.mean(sd_dT).tolist()],
    ...
    def getSD(self) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        Returns:
            tuple: A tuple containing:
                ...
                - sd_dR (np.ndarray): Standard deviation of geodesic distance.
                - sd_dT (np.ndarray): Standard deviation of euclidean distance. 
        """
        geodesic_array = np.asarray(self.geodesic)  # Shape: (num_batches, 2)
        if self.success_idx:
            ...
            sd_dR = np.abs(geodesic_array[np.asarray(self.success_idx), 0]).std(axis=0)
            sd_dT = np.abs(geodesic_array[np.asarray(self.success_idx), 1]).std(axis=0)
        else:
            loss_r = np.abs(np.asarray(self.loss_r)).std(axis=0)
            loss_t = np.abs(np.asarray(self.loss_t)).std(axis=0)
            sd_dR = np.abs(geodesic_array[:, 0]).std(axis=0)
            sd_dT = np.abs(geodesic_array[:, 1]).std(axis=0)
        return loss_r, loss_t, sd_dR, sd_dT
    ...
    def geodesic_distance(self,x:torch.Tensor,)->list:
        """
        geodesic distance and euclidean distance for evaluation
        ...
        # Compute the rotation error (geodesic distance)
    ...
    def save_results(self):
        ...
        json_filename = os.path.join(
            self.config.dataset_config.results_path, 
            "results_" + '_' + self.config.dataset + '_' + 
            self.config.dataset_config.distribution + '_' + str(self.config.dataset_config.max_rot_error) + '_' + 
            str(self.config.dataset_config.max_trans_error) +
            ".json"
        )
...
class MultiLayerCalibEval(object):
    def __init__(self, config, num_layers=3, translation_threshold=None, rotation_threshold=None):
        self.config = config
        self.num_layers = num_layers
        # Create one evaluator per layer.
        self.evaluators = {layer: CalibEval(config, translation_threshold, rotation_threshold)
    ...
    def add_batch(self, layer, gt_tf, pred_tf, idx=None, return_results=False):
        if layer not in self.evaluators:
            raise ValueError(f"Layer {layer} is not valid. Valid layers: 0 to {self.num_layers - 1}.")
        self.evaluators[layer].add_batch(gt_tf, pred_tf, idx, return_results)

    # def get_stats(self):
    #     stats = {}
    #     for layer, evaluator in self.evaluators.items():
    #         stats[layer] = evaluator.get_stats()
    #     return stats

    def save_all_results(self, output_file):
        combined_results = {}
        for layer, evaluator in self.evaluators.items():
            combined_results[f"layer_{layer}"] = evaluator.get_results()
        
        
        combined_results.update({"dataset": self.config.dataset + self.config.dataset_config.version ,
            "model": self.config.dataset_config.model,
            'translation': self.config.dataset_config.max_trans_error,
            'rotation': self.config.dataset_config.max_rot_error,
            'distribution': self.config.dataset_config.distribution})
...
#         self.geodesic = [] # List to store geodesic distances
#         self.success_idx = [] # List to store indices of successful calibrations
#         self.gt = []       # List to store ground truth values
#         self.pred = []     # List to store predicted values
#         self.reset()       # Reset the lists for a fresh start
...
#         self.loss_r.extend(err_euler_angles.tolist())
#         self.loss_t.extend(err_trans.tolist())
#         self.gt.extend(torch.cat((gt_euler_angles, gt_trans), dim=1).tolist())
#         self.pred.extend(torch.cat((pred_euler_angles, pred_trans), dim=1).tolist())
#         self.geodesic.append(self.geodesic_distance(error))
...
#             - distribution: Distribution type used in the dataset configuration.
#             - iter: Number of inner iterations for the model.
#             - gt_calib: Ground truth calibration values.
#             - pred_calib: Predicted calibration values.
#             - error_calib: Concatenated loss values for rotation and translation errors.
#             - mean_error: Mean error values for rotation, translation, and ground truth.
#             - sd: Standard deviation of the rotation and translation errors.
...
#         evaluator = CalibEval(config=None, translation_threshold=translation_threshold, rotation_threshold=rotation_threshold)
 
#         # Create dummy ground truth and predicted transformation matrices
#         gt_tf = torch.eye(4).unsqueeze(0)  # Ground truth transformation (batch size 1)
#         pred_tf = torch.eye(4).unsqueeze(0)  # Predicted transformation (batch size 1)
 
#         # Apply a small translation and rotation to the predicted transformation
#         pred_tf[0, :3, :3] = torch.tensor([[0.9998, -0.0174, 0.0000],
#                                         [0.0174,  0.9998, 0.0000],
#                                         [0.0000,  0.0000, 1.0000]])  # Small rotation
#         pred_tf[0, :3, 3] = torch.tensor([0.1, 0.1, 0.1])  # Small translation
 
#         # Add batch to evaluator
#         evaluator.add_batch(gt_tf, pred_tf)
 
#         # Retrieve and print statistics
#         loss_r, loss_t, geodesic = evaluator.get_stats()
#         print("Mean Rotation Error:", loss_r)
#         print("Mean Translation Error:", loss_t)
#         print("Mean Geodesic Distance:", geodesic)
 
#         # Compute and print standard deviations
#         loss_r_sd, loss_t_sd = evaluator.getSD()
#         print("Rotation Error Std Dev:", loss_r_sd)
#         print("Translation Error Std Dev:", loss_t_sd)
