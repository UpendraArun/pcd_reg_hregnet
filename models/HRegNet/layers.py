...
def get_positive_negative_pairs(embedding1, embedding2, k=2):
    
    B, N, C = embedding1.shape

    # Normalize embeddings for cosine similarity
    emb1_norm = F.normalize(embedding1, dim=-1)  # [B, N, C]
    emb2_norm = F.normalize(embedding2, dim=-1)  # [B, N, C]

    # Compute pairwise similarity
    similarity_matrix = torch.matmul(emb1_norm, emb2_norm.transpose(1, 2))  # [B, N, N]

    # Get top-k matches (positive pairs)
    _, pos_indices = torch.topk(similarity_matrix, k=k, dim=-1)  # [B, N, k]

    #print(f"embedding2 shape: {embedding2.shape}")
    #print(f"pos_indices shape: {pos_indices.shape}")

    # Gather positive pairs
    pos_pairs_emb1 = embedding1.gather(1, pos_indices.expand(-1, -1, C))  # [B, N, C]
    pos_pairs_emb2 = embedding2.gather(1, pos_indices.expand(-1, -1, C))  # [B, N, C]

    # Combine embeddings for positive pairs
    pos_pairs = torch.cat([pos_pairs_emb1, pos_pairs_emb2], dim=2)  # [B, P, 2, C]

    # Create all pair indices
    all_indices = torch.arange(N, device=embedding1.device).unsqueeze(0).repeat(B, 1)  # [B, N]

    # Generate mask for negative pairs (exclude positive indices)
    neg_indices = torch.ones((B, N, N), device=embedding1.device, dtype=torch.bool)  # Initialize all True
    neg_indices.scatter_(2, pos_indices, False)  # Set True to False for positive pairs

    # Expand embeddings for masked_select
    expanded_emb1 = embedding1.unsqueeze(2).expand(-1, -1, N, -1)  # Shape: [B, N, N, C]
    expanded_emb2 = embedding2.unsqueeze(1).expand(-1, N, -1, -1)  # Shape: [B, N, N, C]

    # Masked selection for negative pairs
    neg_emb1 = expanded_emb1.masked_select(neg_indices.unsqueeze(-1)).view(-1, C)  # Shape: [B * N_neg, C]
    neg_emb2 = expanded_emb2.masked_select(neg_indices.unsqueeze(-1)).view(-1, C)  # Shape: [B * N_neg, C]

    # Stack negative pairs
    neg_pairs = torch.stack([neg_emb1, neg_emb2], dim=1).view(B, -1, 2, C)  # Shape: [B, N_neg, 2, C]
...
class CoarseReg(nn.Module):
    ...
    def forward(self, src_xyz, src_desc, dst_xyz, dst_desc, src_weights, dst_weights):
        ...
        if self.use_neighbor:
            ...
            #pos_pairs, neg_pairs = get_positive_negative_pairs(src_nbr_desc, dst_nbr_desc,2)
