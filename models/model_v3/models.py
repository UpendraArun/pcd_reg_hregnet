...
from .layers import KeypointDetector, DescExtractor, CoarseReg, FineReg1, FineReg2
from .layers import WeightedSVDHead, Regression_6dR_3dt_Head, RegressionHead

from pytorch3d.transforms import axis_angle_to_matrix, rotation_6d_to_matrix
from transform.rodrigues import SO3
...
class Model_V3(nn.Module):
    ...
    def forward(self, src_points, dst_points):
        ...
        # Regression head: Layer 3
        r3, t3 = self.regression_head(src_feats['xyz_3'], src_xyz_corres_3, src_dst_weights_3)
        R3 = self.so3.exp(r3)
        ...
        # Regression head: Layer 2
        r2_, t2_ = self.regression_head(src_xyz_2_trans, src_xyz_corres_2, src_dst_weights_2)
        R2_ = self.so3.exp(r2_)
        ...
        # Regression head: Layer 1
        r1_, t1_ = self.regression_head(src_xyz_1_trans, src_xyz_corres_1, src_dst_weights_1)
        R1_ = self.so3.exp(r1_)
        ...
        ret_dict['dst_xyz_2'] = dst_feats['xyz_2']#.to(torch.float16) # chamfer loss - dst points
